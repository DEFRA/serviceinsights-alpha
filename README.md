# serviceinsights-alpha

[![Security Rating](https://sonarcloud.io/api/project_badges/measure?project=DEFRA_serviceinsights-alpha&metric=security_rating)](https://sonarcloud.io/summary/new_code?id=DEFRA_serviceinsights-alpha)
...
> compatibility fEx: `docker build --platform=linux/arm64 --no-cache --tag serviceinsights-alpha`

Build:

```bash
docker build --target development --no-cache --tag serviceinsights-alpha:development .
```

Run:

```bash
docker run -p 3000:3000 serviceinsights-alpha:development
```
...
```bash
docker build --no-cache --tag serviceinsights-alpha .
```

Run:

```bash
docker run -p 3000:3000 serviceinsights-alpha
```
